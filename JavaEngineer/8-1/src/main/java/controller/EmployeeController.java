 ...
 */
 ...
 EmployeeBean employeeBeanData = new EmployeeBean();
 employeeBeanData = EmployeeService.search(id,password);

 
  // 問④ nullの部分に適切な引数をセットする。
 request.setAttribute("EmployeeBean",employeeBeanData);
 
 } catch (Exception e) {
 e.printStackTrace();
 } finally {
 ServletContext context = this.getServletContext();
// サーブレットJSPを表示するためのインターフェイス
